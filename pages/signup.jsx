			...
			<div>
				...
				<div className="wtWpr">
					<form className="wtMem" onSubmit={handleSubmit}>
						<div>
							<input value={unme} onChange={(e) => setUnme(e.target.value)} name="wl_1" type="text" placeholder="username" />					
						</div>
						<div>
							<input value={eml} onChange={(e) => setEml(e.target.value)} name="wl_2" type="email" placeholder="email" />					
						</div>
						<div>
							<input value={pwd} onChange={(e) => setPwd(e.target.value)} type="password" placeholder="password" />					
						</div>
						<button className="btn1 w100 pad10" style={{padding: '10px'}}>Signup</button>
					</form>
					<GFooter/>
				</div>
			</div>
